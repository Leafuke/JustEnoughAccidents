...
JEA 0.2.0 不支持专用服务器。专用服务器加载时只会记录一次禁用日志，不扫描玩家或发起备份。
...
- JEA 快照与普通 FolderRewind 备份共用同一个 `KeepCount`。频繁触发可能使较旧归档被正常清理；0.2.0 不提供独立配额、固定或保护槽位。

## 当前不包含

定时安全检查点、固定最近安全点、致命伤害事件、普通燃烧、TNT/末地水晶/床/重生锚、宠物死亡、自动还原和专用服务器支持均不在 0.2.0 范围内。
